...
export class ContextMenu {
   ...
   * @param tab 
   */
  private onClickListener(info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) {
  ...
  private createMenu(option: Menu): string {
    ...
    } as chrome.contextMenus.CreateProperties).toString()
  ...
  private async createUAMenus(url: URL, tab?: chrome.tabs.Tab): Promise<Menu[]> {
        ...
        return {
          ...
          children: Object.keys(list).map(browser => {
            ...
            return {
              ...
              click: (_, tab) => {
                this.selectUA({ tab, domain, name: uaName, value: list[browser] })
              },
            }
          }),
        }
      }) as Menu[],
      {
        ...
        children: customUA.map(name => {
          return {
            ...
            click: (_, tab) => {
              this.selectUA({ tab, domain, name, value: this.settings!.customUA[name] })
            },
          }
        }),
      },
