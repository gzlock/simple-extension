...
import {getCookies, getCookies2} from '../utils/cookies';
...
export class ContextMenu {
  ...
  private async __saveCookies2(
      hostname: string,
      name: string,
      cookies: Cookies,
  ) {
    const domain = this.settings.domains[hostname] ||
        new Domain();
    this.settings.domains[hostname] = domain;
            ...
            chrome.scripting.executeScript(
                {
                  ...
                  func: function() {
                    return prompt(
                        chrome.i18n.getMessage('prompt_save_cookies'));
                  },
                }, results => {
                  const name = results[0]?.result;
                  if (!name || isEmpty(name)) return;
                  this.__saveCookies2(url.hostname, name, cookies);
                });
          }
        },
      }];
