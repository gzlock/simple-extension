  ...
  <div>
    ...
    >
      <div style="display: flex">
        <el-input
          v-model="form.name"
          :placeholder="ui.hint_name"
        />
        <el-input
          v-model="form.value"
          :placeholder="ui.hint_value"
        />
        <el-button
          type="success"
          @click="add"
    ...
    <el-dialog
      v-model="modify.showDialog"
      :title="ui.dialog_modify_ua_title"
    >
      <el-form :model="form">
...
export default {
  ...
  methods: {
    async add() {
      if (isEmpty(this.form.name)) return alert(ui.alert_title_not_empty)
    ...
    async modifyUA() {
      ...
      Object.values(this.domains).forEach((domain: Domain) => {
        ...
        if (domain.ua?.selected == name) domain.ua!.value = value
      })
      this.updateSettings()
